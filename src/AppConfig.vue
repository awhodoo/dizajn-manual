<template>
	<div id="layout-config" :class="containerClass">
		...
		<div class="layout-config-content">
			...
			<div class="p-formgroup-inline">
				<div class="field-radiobutton">
					<RadioButton id="md-light" name="layoutColorMode" value="md-light" v-model="themeScheme" @change="changeThemeScheme('md-light'); changeThemeColor('indigo')" />
					<label for="md-light">MD Light</label>
				</div>
				<div class="field-radiobutton">
					<RadioButton id="union" name="layoutColorMode" value="union" v-model="themeScheme" @change="changeThemeScheme('union'); changeThemeColor('light')" />
					<label for="union">Union</label>
				</div>
			</div>

			<h6>Primary Color</h6>
			<div v-if="['md-light','md-dark','mdc-light','mdc-dark'].includes(themeScheme)" class="flex">
				<div style="width:2rem;height:2rem;border-radius:6px" class="mr-3 bg-indigo-500 cursor-pointer" @click="changeThemeColor('indigo')"></div>
				<div style="width:2rem;height:2rem;border-radius:6px" class="mr-3 bg-purple-900 cursor-pointer" @click="changeThemeColor('deeppurple')"></div>
			</div>
			<div v-if="['union'].includes(themeScheme)" class="flex">
				<div style="width:2rem;height:2rem;border-radius:6px" class="mr-3 bg-gray-300 cursor-pointer" @click="changeThemeColor('light')"></div>
				<div style="width:2rem;height:2rem;border-radius:6px" class="mr-3 bg-gray-900 cursor-pointer" @click="changeThemeColor('dark')"></div>
			</div>
		</div>
	</div>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			changeTheme() {
				let theme = localStorage.getItem("themeScheme") + '-' + localStorage.getItem("themeColor");
				console.log(theme);
